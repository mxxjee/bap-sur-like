...
1. PC에서 위 정적 서버를 `8000` 포트로 실행합니다.
2. Windows에서 `ipconfig`를 실행해 PC의 내부 IPv4 주소를 확인합니다.
3. 휴대폰과 PC를 같은 Wi-Fi 네트워크에 연결합니다.
4. 휴대폰 브라우저에서 `http://PC내부IP:8000`에 접속합니다. 예: `http://192.168.0.10:8000`

접속되지 않으면 정적 서버가 외부 접속을 허용하는 주소에 바인딩되어 있는지 확인하세요. Windows 방화벽이 Python 또는 Node.js의 `8000` 포트 연결을 차단할 수 있으므로, 신뢰할 수 있는 개인 네트워크에서만 해당 앱이나 포트의 인바운드 연결을 허용해야 합니다.
